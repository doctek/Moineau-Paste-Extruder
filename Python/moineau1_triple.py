...
Generate dxfs of Moineau pump stator and rotor profiles
...
This version creates a three lobe stator.
...
def yhypo(t):
    return 5*math.sin(t)- math.sin(5*t)
	
#xepi(th) := 5*cos(th) - cos(5*th) $
def xepi(t):
    return 7*math.cos(t)-math.cos(7*t)

#yepi(th) := 5*sin(th) - sin(5*th) $
def yepi(t):
    return 7*math.sin(t)- math.sin(7*t)
	
def x(t):
	if t >= 0 and t < math.pi/3:
		return xhypo(t)
	elif t >= math.pi/3 and t < 2*math.pi/3:
		return xepi(t)
	elif t >= 2*math.pi/3 and t < math.pi:
		return xhypo(t)
